...
class nnUNetTrainer_no_rotation(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        self.tb_writer = None
        if self.local_rank == 0 and SummaryWriter is not None:
            self.tb_writer = SummaryWriter(log_dir=join(self.output_folder, "tensorboard"))
    ...
    def on_epoch_end(self):
        ...
        if self.local_rank == 0 and self.tb_writer is not None:
            ...
            self.tb_writer.add_scalar("loss/train", float(log["train_losses"][-1]), epoch)
            self.tb_writer.add_scalar("loss/val", float(log["val_losses"][-1]), epoch)
            self.tb_writer.add_scalar("dice/mean_fg", float(log["mean_fg_dice"][-1]), epoch)
            self.tb_writer.add_scalar("learning_rate", float(self.optimizer.param_groups[0]["lr"]), epoch)
            for idx, dice in enumerate(log["dice_per_class_or_region"][-1], start=1):
                self.tb_writer.add_scalar(f"dice/class_or_region_{idx}", float(dice), epoch)
